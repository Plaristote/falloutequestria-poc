...
class CasinoBodyguard extends CharacterBehaviour {
  ...
  followTheBoss() {
    if (this.model.actionQueue.isEmpty()) {
      ...
      const scriptBoss = boss.getScriptObject();

      if (!scriptBoss.routineComponent.isActiveRoutine("goToSleep")) {
  ...
  onActionQueueCompleted() {
    if (this.killAfterActions) {
      this.killAfterActions.takeDamage(999, this.model);
      delete this.killAfterActions;
    }
